...
class DbConnectionsView extends WorkView {
	...
	createSelectionPanel(builder, target) {
		...
		this.DBComp = UIBuilder.createDomElement<SimpleCrudComp>(SimpleCrudComp.TagName)
			.build(builder)
			.appendTo(target)
	...
	getCurrentConnection(): DbConnectionDef {
	...
	readDataFromView(connection?: DbConnectionDef) {
